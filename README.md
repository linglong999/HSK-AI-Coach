...
**260 个测试全部 PASS** 即说明链路完整（识别/讲解/图谱/降级兜底/前端服务层，全部 mock，不调 LLM 不花钱）。注：早期版本 README 曾标 "32"——那是首个迭代的初始基线，随功能演进已扩至 **260**（含 0.18 契约接缝、0.19 profile/深链、0.20 供应商/BYOK 等模块级测试），口径以下行数字为准。
...
零依赖 HTTP 服务（仅标准库），原生 JS+SVG 单页，**无需安装前端依赖**。**v2（前端对话画布 + 认知地图）**：左侧以卡片流呈现「偏误卡 → 费曼讲解卡 → 学习成果卡」，卡片分支动作全部锚定图谱权威边（无权威边置灰）；侧栏「◈ 认知地图」全屏呈现掌握度 × HSK 等级分布与复习队列。自由对话（`/api/dialog`，planner 唯一入口）需 LLM Key：未配置任何供应商时前端直接报错并给出配置指引（fail-loud，不静默降级）——两种配置方式：① `.env` 填 `DEEPSEEK_API_KEY` 后重启；② 界面「设置 → 模型密钥」添加任意 OpenAI 兼容供应商（即时生效）。仅用确定性纠错闭环（`/api/process`）时可不配 Key，识别自动降级为规则回退、一行不崩（见 "降级策略"）。API 采用**契约化 JSON**（对话契约见 `datasets/docs/0.17-统一能力契约与架构总纲.md`，纠错契约见 `datasets/docs/JSON-接缝契约-v1.md`，前端 v2 见 `datasets/docs/0.19-前端壳v2改造.md`）。
...
├── tests/                       # 260 个测试（免 Key，全 mock；含 0.19 profile/深链 11 个、0.20 providers 20 个）
