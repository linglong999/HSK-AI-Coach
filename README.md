...
**285 个测试全部 PASS** 即说明链路完整（识别/讲解/图谱/降级兜底/前端服务层，全部 mock，不调 LLM 不花钱）。注：早期版本 README 曾标 "32"——那是首个迭代的初始基线，随功能演进已扩至 **285**（含 0.18 契约接缝、0.19 profile/深链、0.20 供应商/BYOK、0.21 教学语言分层等模块级测试），口径以下行数字为准。
...
**核心能力栈**：`多智能体自主编排（planner ReAct）` · `统一 Skill 注册表（9 技能）` · `RAG 语料检索` · `偏误图谱数据层` · `费曼讲解/复述验证引擎` · `多轮记忆 + 写回账本` · `评测闭环（黄金集 + 真实实跑）` · `系统工程（降级兜底/防漂移/幂等）`
...
├── tests/                       # 285 个测试（免 Key，全 mock；含 0.19 profile/深链 11 个、0.20 providers 20 个、0.21 双语 25 个）
