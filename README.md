...
> **用大白话讲：** 这不是一个套壳 API。它是一套**自主编排的学习 Agent**——你输入一句中文，它自己判断"这句错没错、错在哪、卡在了哪个知识点"，再用**费曼教学法**（让你用自己的话复述来检验真懂）把正确用法讲明白，最后把这次的错误沉淀进一张"偏误知识图谱"，过几天还会在合适的时间主动拉你来复习。错的越多，它越了解你；历史越长，复习越精准。全程每一步都有**可复跑的数字**背书——识别精准率 100%、自由对话技能命中 80%+，而不是一句"效果不错"的空话。

- **零第三方依赖**：运行时只用 Python 标准库，clone 即跑
- **API Key 自备**：默认 DeepSeek（已评测校准）；也可在界面「设置 → 模型密钥」直接添加任意 OpenAI 兼容供应商（Qwen/GLM/Kimi/Ollama…），即时生效免重启
...
**385 个测试全部 PASS** 即说明链路完整（识别/讲解/图谱/降级兜底/前端服务层，全部 mock，不调 LLM 不花钱）。注：早期版本 README 曾标 "32"——那是首个迭代的初始基线，随功能演进已扩至 **385**（含 0.18 契约接缝、0.19 profile/深链、0.20 供应商/BYOK、0.21 教学语言分层、0.22 L1 母语迁移归因＋场景对话＋介入时机/个性化等模块级测试），口径以下行数字为准。
...
零依赖 HTTP 服务（仅标准库），原生 JS+SVG 单页，**无需安装前端依赖**。**v2（前端对话画布 + 认知地图）**：左侧以卡片流呈现「偏误卡 → 费曼讲解卡 → 学习成果卡」，卡片分支动作全部锚定图谱权威边（无权威边置灰）；侧栏「◈ 认知地图」全屏呈现掌握度 × HSK 等级分布与复习队列。**双语教学（0.21）**：界面与讲解默认英文脚手架、中文内容载体不翻译（对标 HelloChinese 等主流产品的「教学层母语 + 内容层目标语」策略），「设置 → 语言」一键切换 English/中文。**场景对话（0.22）**：侧栏「📣 场景对话」预置咖啡厅/问路等生活场景，选卡即进入沉浸练习，教学层只带节奏不逐句纠错。**个性化（0.22）**：「设置 → 个性化」定制 AI 回复风格/称呼/人设/自定义指令/打断上限，服务端确定性执行介入时机（流利时静默记录、求助才阻断讲解）。自由对话（`/api/dialog`，planner 唯一入口）需 LLM Key：未配置任何供应商时前端直接报错并给出配置指引（fail-loud，不静默降级）——两种配置方式：① `.env` 填 `DEEPSEEK_API_KEY` 后重启；② 界面「设置 → 模型密钥」添加任意 OpenAI 兼容供应商（即时生效）。仅用确定性纠错闭环（`/api/process`）时可不配 Key，识别自动降级为规则回退、一行不崩（见 "降级策略"）。API 采用**契约化 JSON**（对话契约见 `datasets/docs/0.17-统一能力契约与架构总纲.md`，纠错契约见 `datasets/docs/JSON-接缝契约-v1.md`，前端 v2 见 `datasets/docs/0.19-前端壳v2改造.md`）。
...
**核心能力栈**：`多智能体自主编排（planner ReAct）` · `统一 Skill 注册表（9 技能）` · `RAG 语料检索` · `偏误图谱数据层` · `费曼讲解/复述验证引擎` · `多轮记忆 + 写回账本` · `L1 母语迁移归因` · `场景对话` · `介入时机 + 个性化` · `评测闭环（黄金集 + 真实实跑）` · `系统工程（降级兜底/防漂移/幂等）`
...
- **介入时机确定性执行（0.22）**：流利时对高置信偏误**静默记录**（任务型教学：能交流就不打断），连续出错才轻介入、求助才阻断讲解，打断上限由服务端强制执行不靠 LLM 自觉；预扫识别照常喂图谱，教学层语调与评估层采集解耦

## 评测指标（终态）

> **结果小结**：识别精准 100%，自由对话技能命中 80%+、评估合格率 93%+，
> 这是由黄金评测集 + 多轮真实实跑共同背书、可复现的系统。
...
│   ├── providers.py            # 模型供应商存储/掩码/连通测试（0.20）
...
├── tests/                       # 385 个测试（免 Key，全 mock；含 0.19 profile/深链 11、0.20 providers 20、0.21 双语 25、0.22 迁移 25+场景 25+介入/个性化 50）
...
- [x] 0.19: 前端壳 v2 卡片画布 —— 卡片画布（分支动作锚定图谱权威边）/ 认知地图独立视图 / 智慧批注 / URL 深链 / AI 学习报告卡 / 孟菲斯+暖橙双主题（详见 `datasets/docs/0.19-前端壳v2改造.md`）
- [x] 0.20: 模型密钥 —— 界面内添加任意 OpenAI 兼容供应商（即时生效免重启），输入框按会话切换模型；json_mode 400 自动降级兜底（详见 `datasets/docs/0.20-模型密钥与模型选择.md`）
- [x] 0.21: 教学语言分层 —— 教学层英文脚手架 + 内容层中文载体，界面/讲解双语（默认英文），「设置 → 语言」切换
- [x] 0.22: 习得伙伴转向 —— ①L1 母语迁移归因（`transfer_rules_en.json`+`engine/transfer.py`，假设仅供讲解不污染图谱）、②场景对话技能（`scenarios_en.json`+`engine/scenarios.py`，教学层语调与评估层采集解耦）、③介入时机 + 个性化（`engine/intervention.py` 确定性分档 + `engine/persona.py`+`POST /api/profile`；详见 `datasets/docs/0.22-习得伙伴转向-设计稿.md`，**三个方向全部执行完毕，测试 385 全绿**）
- [ ] M11: 参数标定（aging/mastery 等间隔复习参数）+ 评测集扩集（汉字/词汇/语用）

> **里程碑编号说明（演进对照，勿混淆）**：上表中 `M1–M10/M11` 是项目早期预定的原始里程碑，
> 定义了能力域（识别/讲解/复述/图谱等）；0.17 架构反转后，Agent 化实现以
> `agent-design.md`（本地知识库附件）为 **M1–M9 权威本体**（技能化/注册表/内核/评估/RAG/记忆×图谱/工具），
> 具体接线逐项见 `datasets/docs/0.18-M1-M9-接入清单.md`。两套编号是同一演进的不同阶段记号，
> 当前真身是 Agent 化一套（赋能在对话/检索/图谱），原始 M 编号保留作历史设计坐标。
