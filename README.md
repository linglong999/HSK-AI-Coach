...
<p align="center">
  <a href="#-快速开始">快速开始</a> · <a href="#-核心能力">核心能力</a> · <a href="#-关键设计决策">设计决策</a> · <a href="#-效果核验">效果核验</a> · <a href="#-架构">架构</a> · <a href="#-roadmap">Roadmap</a>
</p>
...
## 🎯 它解决什么问题 / 适合谁 / 不做什么
中文作为二语的学习者，练笔和口语后最缺的是**即时、能讲清原因的反馈**：传统批改要等老师、通用大模型给答案却不解释系统性成因、更糟的是"一开口就被逐句纠正"打击表达欲。

**目前设计给：** 以中文为第二语言、HSK 1–4 的中级学习者（英语母语起步），以及想批量改学生作业的教师。

**当前支持：** 简体中文 · 句子/短文纠错 · 自由对话与场景练习 · 初-中级语法/词汇反馈 · 英文/中文双语教学界面 · 图片/文档 OCR 进纠错链。
...
## 🎭 关键设计决策（为什么这样设计）
1. **宁可漏判，也不误报。** 教学场景里"教错"比"不教"更有害——误报会污染学习者已有认知。干净句误报确定性护栏压到 **0%**，代价是少数边界句可能不纠。
2. **让学习者自己复述，而不是看一遍答案。** 复述才能检验真懂；连续 2 次不过降难度再讲。代价：互动更慢、对低容错学习者更累。
3. **能交流就不打断。** 流利时对高置信偏误静默记录，只有求助/卡壳/连续出错才介入，打断上限服务端强制。来自任务型语言教学原则。
4. **教学层用母语，内容层用目标语。** 英文解释规则、中文承载例句，降低认知负担。
5. **图谱是唯一权威，AI 只能选路径不能改数据。** 纠错结论沉淀进确定性偏误图谱，避免模型漂移、结果可复现。代价：架构更重，模型自由度受限。
6. **每个决策都有数字。** 每块能力配黄金评测集 + 真实模型实跑，指标都能复跑核验。
...
- **M1–M10 · 能力底座** — 识别→讲解→复述→图谱→前端壳依次打通并逐项评测。先验证"纠错这一件事能否做准做稳"，再做交互。
- **0.17 · 架构反转** — 固定流程改为 planner 自由 ReAct + 技能注册表。学习者问题千人千面，固定流水线接不住，改为"自由调度 + 数据写约束"。
- **0.19/0.20 · 前端壳 + 模型密钥** — 卡片画布、认知地图、AI 学习报告；界面内加任意兼容供应商。
- **0.21 · 教学语言分层** — 英文脚手架 + 中文内容载体。
- **0.22 · 习得伙伴转向** — L1 母语迁移归因、场景对话、介入时机 + 个性化。
- **P0.7 · OCR+RAG** — 图片/PDF 进纠错链，OCR 校正语料入检索索引。
- **P0.18 · 效果度量** — 4 指标口径文档 + 评价面板，指标随使用自动更新。

规划中：
- [ ] **M11 · 复习参数标定 + 评测扩集** — 把 aging/mastery 等间隔复习参数标定到有证据的范围，扩到汉字/词汇/语用评测。当前复习队列规则有确定性保证但参数未经验证，需要数据而不是拍脑袋。
