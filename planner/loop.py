...
GLOBAL_RULES = (
    ...
    "5) 某知识点**首次**讲解完成后，主动邀请学习者用自己的话复述一遍（费曼式检验，等其复述后调 verify_retell）；"
    "同一知识点的重复巩固轮次不要主动邀请复述，改为用练习自然带过，或仅在学习者主动提出时邀请。\n"
...
GLOBAL_RULES_EN = (
    ...
    "the final text.\n"
    "5) After a knowledge point's FIRST explanation, invite the learner to restate it "
    "in their own words (Feynman check; call verify_retell once they do). For repeat / "
    "consolidation rounds of the same point, do NOT proactively invite a restate — "
    "let practice carry it, or invite only if the learner asks.\n"
