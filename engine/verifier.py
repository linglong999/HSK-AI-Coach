...
# 意图（E-1=a 拍板，P0.19 ⑭）：解析"3 要点"与"0.8 覆盖率"的关系——
# 要点少时近似全中要求（1/1=1.0、2/3=0.67 判 partial），刻意严格；要点 ≥5 时
# 0.8 阈值自然恢复弹性。不放宽，仅此显式注释锁定意图。
PASS_RATIO = 0.8
...
- 覆盖（原理型）：要点「'坐'表乘坐交通工具」，复述「'坐公交''坐地铁'都是"坐"+交通工具，表示乘用」→ true（讲清了规则的原理/适用范围，体现了该要点语义，即使没有另造句子）。
