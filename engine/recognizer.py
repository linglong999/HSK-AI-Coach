...
6. 每个偏误给出 knowledge_point_id（从清单中选最贴切者）。**清单中存在对应项时必填该 id，不得留空**；留空仅限清单确实没有贴近的对应项。给出错误 id 比留空更糟——宁可留空也不要臆造清单外的 id。
...
输入：老师，你几岁？（对象为成年老师）
输出：{{"fragment":"你几岁","correction":"您多大年纪","type":"语用","type_confident":true,"confidence":0.8,"knowledge_point_id":""}}
...
例4（语法/语序：数量词"很多"作定语却后置）
判定条件：句中"很多"修饰一个此前已明确的名词，却置于句末或谓语之后造成定语后置（如"我想要奶茶，很多"→ 应"很多奶茶"）；若"很多"作谓语表数量（"这家店人很多"）或作独立表语，则位置合法，不报。
输入：我想要奶茶，很多。
输出：{{"fragment":"很多","correction":"很多奶茶","type":"语法","type_confident":true,"confidence":0.75,"knowledge_point_id":"kp-zhuangyu-chezhi"}}
反例（合法，不报）：这家店人很多，所以排了很长的队。（"很多"作谓语表数量）

例5（低置信示例·语体偏好：口语词用于书面/正式语境）
说明：本条不求你有把握拿高置信——它演示"语体偏好类"偏误走**待确认低置信档**（confidence 0.5~0.7），且清单内确无对应知识点时 knowledge_point_id 置空（不是不填，而是这里确实没有）。该类判断主观，把握不足宁可低置信也不要高置信瞎报。
判定条件：句子所在语境明显偏正式（书面报告、正式介绍、对长辈谈吐），却用了仅适合口语的非正式语气词/口语词；若语境本就口语，则为合法，不报。
输入：（正式自我介绍）我叫小明，我很高兴认识大家呢。
输出：{{"fragment":"呢","correction":"。","type":"语用","type_confident":false,"confidence":0.6,"knowledge_point_id":""}}
